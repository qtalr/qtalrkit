...
Table: (\#tab:ida-workflow) The inference-based modeling workflow
...
read_csv("data/derived/pelic_transformed_dd.csv") |>
  kbl(col.names = c("Variable", "Name", "Type", "Description"), caption = "Data dictionary for the transformed PELIC/ ELP dataset.") |>
  kable_styling("striped")
```

#### Identify

In Table \@ref(tab:data-dictionary), we can see the variables which will be used to represent lexical sophistication and learner proficiency. The explanatory variables which will be used to represent lexical sophistication are `chars`, `sylls`, `morphs`, and `freq`. The response variable which will be used to represent learner proficiency is `placement`.
...
First step is to get a statistical overview of the dataset. We can use the `skim()` function from the `skimr` package to get a statistical summary of the dataset.
...
I would like to introduce a shortcut to create all three types of visualizations in one plot. The `ggpairs()` function from the `GGally` package creates a matrix of plots for all combinations of variables in the dataset. We can specify the type of plot to create for each combination of variables. Let's create a matrix of plots for the variables of interest.

```{r ggpairs, fig.width=7, fig.height=7, fig.cap="Matrix of plots for the variables of interest."}
...
The plot in Figure \@ref(fig:ggpairs) shows a lot of information. Let's break it down.
...
Now we will analyze the data using the `infer` package. The `infer` package is a framework for conducting statistical inference using simulation-based methods. The steps for using the `infer` package are as follows:
...
2. What is the primary goal of using the `infer` package for simulation-based inference? `r mcq(c("To visualize data distributions", "To perform regression analysis", answer = "To create resampling simulations for hypothesis testing and constructing confidence intervals", "To fit predictive models"))`
...
- Utilizing the `infer` package in R to create resampling simulations for hypothesis testing and constructing confidence intervals.
