---
title: "9. Building predictive models"
pkgdown:
  as_is: true
...
Table: (\#tab:pda-workflow) The predictive modeling workflow
...
get_compressed_data(url = plain_url, target_dir = "vignettes/articles/data/original/actives", confirmed = TRUE)

aes <- readtext::readtext(
  file = "vignettes/articles/data/original/actives",
...
readr::write_csv(aes_df, "vignettes/articles/data/derived/actives/actives_curated.csv")

# Document data dictionary -------------------------------------------
create_data_dictionary(data = aes_df, file_path = "vignettes/articles/data/derived/actives_curated_dd.csv", model = "gpt-3.5-turbo")
```
...
We will then set the base recipe which formally identifies the relationship between the predictor and outcome variables. The `recipe()` function from the `recipes` package is used to create a recipe.
...
```{r tfidf-viz, fig.cap="Top 20 terms by class"}
...
To combine the recipe and the model specification, we will use the `workflows` package. We will use the `workflow()` function and pass `add_recipe(aes_rec)` and `add_model(aes_spec)` as arguments to add the recipe and the model specification to the workflow.
...
```{r aes-a1-conf-mat, fig.cap="Confusion matrix for the model in Approach 1"}
aes_train_fit |>
...
Now we set up the range of values for both the `max_tokens` and `penalty` hyperparameters. The `grid_regular()` function from the `dials` package will allow us to specify a grid of values for each hyperparameter.
...
```{r aes-tune-metrics, fig.cap="Metrics for model tuning in Approach 2"}
...
```{r aes-a2-conf-mat, fig.cap="Confusion matrix for the model in Approach 2"}
aes_final_fit |>
  collect_predictions() |>
...
```{r aes-a2-coefs-viz, fig.cap="Top 25 terms by class"}
aes_coefs_z |>
