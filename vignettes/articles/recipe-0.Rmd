...
knitr::include_graphics(
  c("assets/images/recipe-0/quarto-preview.png")
)
```
...
At the most basic level a Quarto document contains two components: (1) a front-matter section and (2) a prose section. A third component, a code block, can be interleaved within the prose section to add code to the document. Let's look at each of these in turn. 
...
A code block is bound by three backticks `` ``` ``. After the first backticks the curly brackets `{}` allow us to tell Quarto which programming language to use to evaluate (*i.e.* run) in the code block. In most cases this will be R, hence the the opening curly bracket `` `{r}` ``. In this example, R is used as a simple calculator adding `1 + 1`. Here's what this code block produces. 
...
This concludes our introduction to literate programming using Quarto. We have covered the basics here but there is much more to explore. We will introduce more Quarto functionality in the next Recipe.
