...
```{r skim-brown-fam, results="asis", tidy=FALSE}
# Load packages
library(skimr)
...
Now, we can also pipe the results of a `group_by()` and `summarize()` to another function. This can be to say sort, select, or filter the results. It can also be to perform another summary function. It is important, however, to remember that the result of a `group_by()` produces a grouped data frame. Subsequent functions will be applied to the grouped data frame. This can lead to unexpected results of the original grouping is not relevant for the subsequent function. To avoid this, we can use the `ungroup()` function to remove the grouping after the relevant grouped summary statistics have been calculated.
...
Let's calculate the number of observations `n()` grouped by the `genre` variable in the `brown_fam_df` dataset.
...
Let's add `lang_variety` to the grouping and calculate the number of observations `n()` grouped by the `genre` and `lang_variety` variables in the `brown_fam_df` dataset.
...
Another way to create frequency and contingency tables is to use the `tabyl()` function from the `janitor` package [@R-janitor]. Let's calculate the number of observations `n()` grouped by the `genre` variable in the `brown_fam_df` dataset.
...
The `janitor` package includes a variety of `adorn_*()` functions to add additional information to the results of `tabyl()`, including percentages, number of observations, and totals.

- `adorn_percentages()`: add percentages to the results of `tabyl()`
- `adorn_pct_formatting()`: format percentages to include a `%` sign
- `adorn_ns()`: add the number of observations to the results of `tabyl()`
- `adorn_rounding()`: round the results of `tabyl()`
- `adorn_totals()`: add totals to the results of `tabyl()`
...
```{r tabyl-brown-fam-genre-lang-variety-adorn-perc}
# Cross-tabulation for `genre` and `lang_variety`
brown_genre_lang_ct <-
  brown_fam_df |>
  ...
  adorn_ns("front") # add number of observations (in front)

# View
brown_genre_lang_ct
```

You can see that I assigned the results of the cross-tabulation to the object `brown_genre_lang_ct`. It is important to note that the results of `tabyl()` are data frames, albeit with a special class `tabyl`. 

```{r class-brown-genre-lang-ct-inspect}
# Class of `brown_genre_lang_ct`
class(brown_genre_lang_ct)
```

Therefore, we can apply subsequent operations to the results of `tabyl()` as we would any other data frame. However, we must pay attention to the variable types of the results of `tabyl()`. 

```{r class-brown-genre-lang-ct-inspect-variables}
# Variable types of `brown_genre_lang_ct`
glimpse(brown_genre_lang_ct)
```

We see that what look like numeric values in the results of `tabyl()` are actually character values. Just something to be aware of when working with the results of `tabyl()`.
...
````
```{r kable-brown-genre-lang-ct-label-tbl-cap}`r ''`
...
```{r ggplot2-brown-fam-percent-passive-hist-label-fig-cap}`r ''`
