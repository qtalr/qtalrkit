...
In this file we can see that the reference includes information about the type of publication, the title, the authors, the year, the journal, the volume, the number, the pages, and the DOI.
...
With the bibliography file and the reference to the bibliography file in the front-matter section, we can now add citations to our document. To do this, we use the `@` symbol followed by the citation key in the prose section. For example, to cite the `R-tidyverse` reference from the `bibliography.bib` file, we would add `@R-tidyverse` to the prose section as follows:

```
This is a citation to @R-tidyverse.
```
