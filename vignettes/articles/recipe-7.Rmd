...
In Table \@ref(tab:masc-transformed-idealized), we see an idealized version of the dataset we would like to have.
...
Table: (\#tab:masc-transformed-idealized) Idealized version of the MASC dataset
...
3. `r torf(TRUE)` The `count()` function from the `dplyr` package is used to tabulate the values of a variable.
