...
The approach to exploratory analysis is highly iterative. The process is not linear, but rather a cycle of the steps in Table \@ref(tab:eda-workflow). This cycle is repeated until the research question(s) have been addressed.
...
Table: (\#tab:eda-workflow) The exploratory analysis workflow
...
In Lab 8, we will try our hand at exploratory analysis using another dataset.
...
<!-- [ ] return and update for the methods actually used -->
...
```{r sotu-corpus, message=FALSE, warning=FALSE, echo=FALSE}
sotu_df <-
...
```{r sotu-corpus-summary-show, eval=FALSE}
skimr::skim(sotu_df)
```
...
```{r sotu-words-over-time, fig.cap="Number of words per year"}
...
We can see from Figure \@ref(fig:sotu-words-over-time) that the number of words per year varies, sometimes quite a bit. To get a sense of the relationship between the number of words and the year, we can add a linear trend line to the plot. We can do this by adding the `geom_smooth()` function to the plot. We will set the `method` argument to `"lm"` to use a linear model.

```{r sotu-words-over-time-trend-lm}
...
```{r sotu-freq-words-tfidf-top-10}
...
```{r sotu-freq-lemmas-tfidf-top-10}
...
```{r sotu-terms-tfidf-df-political, fig.cap="Distinctiveness of political words over time"}
...
We can see in Figure \@ref(fig:sotu-terms-tfidf-df-political) that the distinctiveness of these lemmas varies over time. Now, in this plot I've used a small span value for the `geom_smooth()` function to get a sense of the more fine-grained changes over time. However, this is may be too fine-grained. We can adjust this by increasing the `span` value. Let's try a `span` value of 0.5.

```{r sotu-terms-tfidf-df-political-smooth-0.5, fig.cap="Distinctiveness of political words over time"}
sotu_terms_tfidf_df |>
  filter(lemma %in% plot_terms) |>
  ggplot(aes(year, tf_idf)) +
...
Figure \@ref(fig:sotu-terms-tfidf-df-political-smooth-0.5) seems to be picking up on some of the more general word usage trends over time.
...
```{r sotu-terms-tfidf-df-political-smooth-0.5-fixed, fig.cap="Distinctiveness of political words over time"}
sotu_terms_tfidf_df |>
  filter(lemma %in% plot_terms) |>
  ggplot(aes(year, tf_idf)) +
  geom_smooth(se = FALSE, span = 0.5) +
...
In Figure \@ref(fig:sotu-terms-tfidf-df-political-smooth-0.5-fixed), we can clearly see that the magnitude of the words "crime" and "terror" are much higher than the other words we happend to select. Furthermore, these words have interesting patterns. In particular, "terror" has two peaks on around 1980 and another around the turn of the century. "Crime" also has two distinctive peaks on in the 1970s and one in the 1990s.
...
```{r sotu-terms-tfidf-sd-based, fig.cap="Distinctiveness of words over time"}
...
library(PsychWordVec)     # for aligning word embeddings
```
...
```{r sotu-create-word-embeddings-show, eval=FALSE}
...
sotu_vec_mods <- read_write_load_word2vec("~/Desktop/sotu")
```

```{r sotu-create-word-embeddings-run, include=FALSE}
# Create word embeddings for each decade --------------------------
# - CBOW better for more common words
# - Skip-gram better for less common words
#   Given the varying sizes of the sub-corpora, Skip-gram might be more suitable as it may capture more nuances in less frequent words.
read_write_load_word2vec <- function(path) {
  txt_files <- dir_ls(path, regexp = "\\.txt$")
  models <- txt_files |> map(\(x) {
    model <- word2vec(x = x, type = c("skip-gram"), dim = 100, split = c(" ", ".?!"))
    bin_file <- str_replace(x, "\\.txt$", ".bin")
    write.word2vec(model, bin_file)
    read.vectors(bin_file)
  })
...
# Read and train the word2vec models
sotu_vec_mods <- read_write_load_word2vec("~/Desktop/sotu")
```

Each word embedding model is a matrix with the words as the rows and the dimensions as the columns as an element of the `sotu_vec_mods` list. Each of these elements have the name of the decade. We can extract an element from a list using the `pluck()` function from the `purrr` package.
...
Now we can align the models to a common space. The reason that the models need to be aligned is that the word embeddings are trained on different corpora. This means that the words will be represented in different spaces. We will use the Orthogonal Procrustes solution to align the models to a common coordinate space. The `PsychWordVec` package [@R-PsychWordVec] has a function, `orth_procrustes()`, that will do this for us. In the process of aligning the models, the models are converted to plain matrices so we will need to convert them back to `VectorSpaceModel` objects.
...
```{r}
# Visualize the change in cosine similarity over time -------------
data.frame(decade = basename(names(differences)), difference = unlist(differences)) |>
  arrange(decade) |>
  ggplot(aes(decade, difference, group = 1)) +
...
# Get the cosine difference for multiple words --------------------

get_cosine_diff(word = c("freedom", "equality", "community"), models = sotu_aligned_mods) |>
  ggplot(aes(decade, difference, group = word)) +
  geom_smooth(se = FALSE, span = 1) +
  labs(title = "Cosine difference over time")
```
...
sotu_pca_df |>
  filter(str_detect(word, "^(freedom|free|community|individual|force|military)_")) |>
