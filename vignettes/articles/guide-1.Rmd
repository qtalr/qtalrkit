...
In this guide, we will explore options for setting up an R environment. We will discuss local, remote, and virtual environments. Each have their own advantages and shortcomings.

::: {.callout}
**`r fontawesome::fa('list-alt')` Outcomes**

- Understand the different options for working with R
...
To get started, install R from [CRAN](https://cran.r-project.org/). You can download the latest version of R for your operating system [here](https://cran.r-project.org/mirrors.html). Once you have installed R, you will need to install an IDE. We recommend [RStudio](https://www.rstudio.com/products/rstudio/download/), a free and open-source IDE for R. RStudio provides a number of features that make it easier to work with R. 

For projects that require collaboration, we recommend using a version control system such as [Git](https://git-scm.com/) and a hosting service such as [GitHub](https:://github.com) or [GitLab](https://gitlab.com). These tools will help you manage your code and collaborate with others.
...
In this guide, we have discussed strategies for working with R. All three options offer unique advantages. In \@ref(ab:env-comparison), we summarize some of the characteristics, benefits, and drawbacks of each option.
