...
#'
#' @param tdm_normalized A normalized term-document matrix (TDM) where each row represents a type and each column represents a document. The values should be the proportions of each type's frequency to its total frequency across all documents.
#'
#' @param corpus_parts A numeric vector containing the proportions of each document in the corpus, which is used to calculate the Deviation of Proportions (DP).
#'
#' @return A numeric vector containing the Deviation of Proportions (DP) for each type in the TDM.
#'
#' @keywords internal
#'
