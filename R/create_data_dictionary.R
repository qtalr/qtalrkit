#' Create a data dictionary for a given data frame.
#'
...
#'
#' @return A data frame containing the variable name, human-readable name, variable type, and description for each variable in the input data frame.
#'
#' @examples
#' \dontrun{
#' data(mtcars)
...
#' @importFrom openai create_chat_completion list_models
#'
#' @export
create_data_dictionary <- function(data, file_path, model = NULL, sample_n = 5, grouping = NULL, force = FALSE) { # nolint
  ...
  if (file.exists(file_path) && !force) stop("File already exists at `file_path`.") # nolint
  ...
  if (is.null(model)) {
    data_dict <-
      tibble(
        variable = names(data),
        name = NA_character_,
        variable_type = purrr::map_chr(data, class),
        description = NA_character_
      )
  } else {
    # Check to see if `model` is one of the available models
    if (!model %in% openai::list_models()[[2]]$id) stop("`model` must be one of the available chat completion models. See `openai::list_models()`.") # nolint

    # Check to see if OPENAI_API_KEY is set
    if (is.na(Sys.getenv()["OPENAI_API_KEY"])) stop("Set your OPENAI_API_KEY environment variable.") # nolint

    # Set the instructions for the prompt
    prompt_instructions <- "I have a dataset I would like you to create a data dictionary for. The information I want is the `variable`, `name` (human readable), `variable_type` (one of 'categorical', 'ordinal', or 'numeric'), and `description`. Here's a small sample of the data for you to work with. In some cases some variables in the dataset may be null. In these cases use the variable name to predict the other information. Please return your data dictionary in `.csv` format. Remember to enclose text in quotes and only return the `.csv` info, no explanations." # nolint
    ...
    # Use openai to generate the descriptions for each of the variables in the
    # `data_sample` data frame. This will be used to create the data dictionary.
    response <-
